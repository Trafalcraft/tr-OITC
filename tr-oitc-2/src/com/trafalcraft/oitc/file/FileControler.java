	...
	public static void loadAllFile(){
		...
		for(String sF : f2.list()){
			if(sF.endsWith(".yml")){
				...
				Bukkit.getLogger().info("§3§lOITC> §b" + "Le fichier "+sF+" a bien été chargé");
			}
		}
	}
	
	public static void loadFile(String arene){
		File f = new File(Main.getPlugin().getDataFolder()+"//arene//" , arene + ".yml");
		arenaList.put(f.getName().replace(".yml", ""), YamlConfiguration.loadConfiguration(f));
		Bukkit.getLogger().info("Le fichier "+ arene +" a bien été chargé");
	}
